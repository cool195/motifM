...
(function($) {
    ...
    /**
     * switchImpr - description
     *
     * @param  { element } Impr description
    ...
    function imprItem() {
        var $ImprItem = $('a[data-impr]');
        if ($ImprItem.length !== 0) {
            pushAjax($ImprItem);
        }
    }
    ...
    $('a[data-clk]').on('click', function(event) {
        // event.preventDefault();
        $.ajax({
            url: $(event.target).data('clk')
        });
    });
})(jQuery);

//# sourceMappingURL=impr.js.map
