        ...
        @if(isset($topic['infos']))
            @foreach($topic['infos'] as $k=>$value)
                ...
                @elseif($value['type']=='title')
                    <!-- 标题 -->
                        <a href="@if($value['skipType']=='1')motif://o.c?from=topic_detail&from_id={{$topicID}}&a=pd&spu={{$value['skipId']}}@elseif($value['skipType']=='2')/collection/{{$value['skipId']}}@elseif($value['skipType']=='3')/topic/{{$value['skipId']}}@elseif($value['skipType']=='4')motif://o.c?from=topic_detail&from_id={{$topicID}}&a=shoppinglist&cid={{$value['skipId']}}@else{{'motif://o.c?from=topic_detail&from_id=' . $topicID . '&a=outurl&url='.urlencode($value['skipId'])}}@endif">
                ...
                @elseif($value['type']=='product')
                        ...
                        @if($value['style']=='box-vertical')
                            ...
                            @if(isset($value['spus']))
                                ...
                                @endforeach
                            @else
                                <a href="@if(!isset($value['skipId']))javascript:;@elseif($value['skipType']=='1')motif://o.c?from=topic_detail&from_id={{$topicID}}&a=pd&spu=@elseif($value['skipType']=='2')/collection/@elseif($value['skipType']=='3')/topic/@elseif($value['skipType']=='4')motif://o.c?from=topic_detail&from_id={{$topicID}}&a=shoppinglist&cid=@endif{{'motif://o.c?from=topic_detail&from_id=' . $topicID . '&a=outurl&url='.urlencode($value['skipId'])}}">
                                    <img class="img-fluid img-lazy"
                                         data-original="{{env('APP_Api_Image')}}/n1/{{$value['imgPath']}}"
                                         src="{{env('CDN_Static')}}/images/product/bg-product@750.png"
                                         alt="">
