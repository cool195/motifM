    ...
    <div class="body-container" style="padding-top: 85px;">
        ...
        <section class="bg-white reserve-height">
            ...
            @elseif($value['type']=='product')
                <div data-impr='{{ config('app.clk_url') }}/log.gif?t=daily.200001&m=H5_M2016-1&pin={{Session::get('user.pin')}}&uuid={{Session::has('user') ? Session::get('user.uuid') : $_COOKIE['uid']}}&v={"action":0,"skipType":1,"skipId":"{{ implode("_", $value['spus']) }}","topicId":{{$topicID}},"expid":0,"ver":"1.0.1","src":"H5"}'></div>
