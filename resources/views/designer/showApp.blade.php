                                ...
                                <a href="javascript:void(0)">
                                    @else
                                        ...
                                        @elseif($value['type']=='title')
                                            {{--标题--}}
                                            <a data-link="@if($value['skipType']=='1')motif://o.c?from=designer_detail&from_id={{$designer['designer_id']}}&a=pd&spu={{$value['skipId']}}@elseif($value['skipType']=='2')/collection/{{$value['skipId']}}@elseif($value['skipType']=='3')/topic/{{$value['skipId']}}@elseif($value['skipType']=='4')motif://o.c?from=designer_detail&from_id={{$designer['designer_id']}}&a=shoppinglist&cid={{$value['skipId']}}@else{{'motif://o.c?from=designer_detail&from_id=' + $designer['designer_id'] + '&a=outurl&url='.urlencode($value['skipId'])}}@endif"
                                               data-impr='{{ config('app.clk_url') }}/log.gif?time={{time()}}&t=designer.400001&m=H5_M2016-1&pin={{Session::get('user.pin')}}&uuid={{Session::has('user') ? Session::get('user.uuid') : $_COOKIE['uid']}}&v={"action":0,"skipType":"{{$value['skipType']}}","skipId":"{{$value['skipId']}}","expid":0,"index":"{{$k}}","version":"1.0.1","ver":"9.2","src":"H5"}'
                                               data-clk='{{ config('app.clk_url') }}/log.gif?time={{time()}}&t=designer.400001&m=H5_M2016-1&pin={{Session::get('user.pin')}}&uuid={{Session::has('user') ? Session::get('user.uuid') : $_COOKIE['uid']}}&v={"action":1,"skipType":"{{$value['skipType']}}","skipId":"{{$value['skipId']}}","expid":0,"index":"{{$k}}","version":"1.0.1","ver":"9.2","src":"H5"}'
                                               href="javascript:void(0)">
...
<script>
    ...
    Jockey.on("action", function (action) {
        ...
        if (action.name == "authInfo") {
            ...
            window.location.href = "/collection/{{$designer['designer_id']}}?f=" + f + "&des=" + $('#followDes').val() + "&wishspu=" + $('#wishspu').val() + "&token=" + action.data.token + "&pin=" + action.data.pin + "&email=" + action.data.email + "&name=" + decodeURIComponent(action.data.name)
        }
